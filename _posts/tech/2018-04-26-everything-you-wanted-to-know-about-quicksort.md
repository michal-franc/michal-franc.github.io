...
First of all - check if there is a need to sort the array. If its lenght is less than equal one then just return the array - Done. There is no need to allocate resources and push thing on to stack with method calls. It's already sorted.
